...
trait DeliteOpsExpIR extends DeliteReductionOpsExp with StencilExp with NestedLoopMappingExp {
  ...
  case class DeliteCollectFlatOutput[A:Manifest, I:Manifest, CA:Manifest](
    override val eV: Sym[A],
    override val sV: Sym[Int],
    override val allocVal: Sym[I],
  ...
  case class DeliteHashCollectElem[K:Manifest,V:Manifest,I:Manifest,CV:Manifest,CI:Manifest,CCV:Manifest](
    ...
    iBufSize: Block[Int],
    numDynamicChunks: Int
  ) extends DeliteHashElem[K,CCV] with DeliteLoopElem[CCV] {
    val mK = manifest[K]
    val mV = manifest[V]
    val mI = manifest[I]
    val mCV = manifest[CV]
  ...
  case class DeliteHashReduceElem[K:Manifest,V:Manifest,I:Manifest,CV:Manifest](
    ...
    numDynamicChunks: Int
  ) extends DeliteHashElem[K,CV] with DeliteLoopElem[CV] {
    val mK = manifest[K]
    val mV = manifest[V]
    val mI = manifest[I]
    val mCV = manifest[CV]
  ...
  /**
    * TODO: DAMIEN: write documentation
    * WHAT I THINK: need to compute first element to initialize the reduce
    * PROBLEM: how can a reduce be a map (CollectAnyMap) ?
    * @param e
    * @tparam A
    * @return
    */
  def loopBodyNeedsStripFirst[A](e: Def[A]) = e match {
    case r:DeliteReduceElem[_] => getCollectElemType(r) match {
  ...
  /**
    * TODO: DAMIEN: write documentation
    * WHAT I THINK: pieces of the result need to be recombined into one collection
    * PROBLEM: could not find it used anywhere in the code
    * DeliteCollectElem => false // because is a fixed size map ? so can directly write to correct place ?
    * @param e
    * @tparam A
    * @return
    */
  def loopBodyNeedsCombine[A](e: Def[A]) = e match {
    case _:DeliteReduceElem[_] => true
    case _:DeliteFoldElem[_,_] => true
    case _:DeliteHashReduceElem[_,_,_,_] => true
    case _:DeliteHashCollectElem[_,_,_,_,_,_] => true
    case _:DeliteHashIndexElem[_,_] => true
    case _:DeliteCollectElem[_,_,_] => false //e.strategy == OutputBuffer
    case _ => false
  }

  def loopBodyNumDynamicChunks[A](e: Def[A]) = e match {
    case e:DeliteLoopElem[_] => e.numDynamicChunks
    case _ => 0
  }
  ...
  /**
    * TODO: DAMIEN: write documentation
    * WHAT I THINK: need to call finalize or not
    * PROBLEM: i don't think this i right
    * @param e
    * @tparam A
    * @return
    */
  ...
  def summarizeBody[A](d: Def[A]) = {
    ...
    // TODO: DAMIEN: understand why we remove stuff
