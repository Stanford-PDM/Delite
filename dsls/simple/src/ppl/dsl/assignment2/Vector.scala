...
trait VectorOpsExp extends VectorOps with DeliteCollectionOpsExp with DeliteStructsExp {
  ...
  private def infix_data[A:Manifest](x: Exp[Vector[A]]): Rep[DeliteArrayBuffer[A]] = field[DeliteArrayBuffer[A]](x, "data")
  ...
  def vectorTimesScalar[A:Manifest:Numeric](x: Exp[Vector[A]], y: Exp[A]) = reflectPure(VectorTimesScalar(x, y))
  ...
  def isVec[A](x: Exp[DeliteCollection[A]])(implicit ctx: SourceContext) = isSubtype(x.tp.runtimeClass, classOf[Vector[A]])  
  ...
  override def unapplyStructType[T:Manifest]: Option[(StructTag[T], List[(String,Manifest[_])])] = {
    val m = manifest[T]
    if (m.runtimeClass == classOf[Vector[_]]) Some((classTag(m), List("data" -> manifest[DeliteArrayBuffer[T]])))
